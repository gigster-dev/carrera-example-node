...
NAMESPACE=gigdemo-$ENVIRONMENT
ENVIRONMENT_FOLDER=gigdemo-$ENVIRONMENT
SERVICE_ROOT=$PROJECT_PATH/$SERVICE_PATH
DOCKERFILE=$SERVICE_ROOT/$DOCKERFILE_PATH
SERVICE_NAME=gigdemo-$SERVICE
SERVICE_TAG=$SERVICE_NAME:$TAG
SECRET_NAME=gigdemo-$SERVICE-secrets
ENCRYPTED_ENV_NAME="${SERVICE_NAME}-secrets"
CONFIGMAP_NAME=gigdemo-$SERVICE-configmap
...
handle_environments() {
  if [ "$ENVIRONMENT" = "uat" ]; then
    PROVIDER_KIND=gcp
    GCP_PROJECT_ID=${GCP_PROJECT_ID:-"gde-gigdemo"}
